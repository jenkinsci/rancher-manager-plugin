...
- **Rancher Manifest Deployment** (`rancherManifest`) — YAML from Git or manual; `POST …?action=apply`; Settle timeout polls Deployment/StatefulSet/DaemonSet/Job until Ready (`waitTimeoutSeconds`, default 300). ConfigMap-only succeeds after apply. Namespace comes from the YAML; the step does not create namespaces
